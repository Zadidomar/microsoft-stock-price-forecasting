...
PROC ARIMA DATA=MSF;
IDENTIFY VAR=X ESACF MINIC;
IDENTIFY VAR=Y ESACF MINIC;
proc print;
RUN;
...
/*ACF for higher lags for testing seasonality*/
PROC ARIMA DATA=MSF;
IDENTIFY VAR=X NLAG=36;
RUN;

/*regression to check the impact of deterministic seasonal component, if any*/
DATA MSF;
SET MSF;
PI=ATAN(1)*4;
S1=SIN(PI/2*T); S2=SIN(PI*T); C1=COS(PI/2*T); C2=COS(PI*T);
RUN;
PROC REG DATA=MSF;
MODEL X = T C1 S1 C2;
...
/* For seasonal unit root of X */
DATA MSF;
SET MSF;
Z=log(X);
W=DIF(Z);
SW=DIF12(W);
SW1=LAG(SW); SW2=LAG(SW1); SW3=LAG(SW2); SW4=LAG(SW3); SW5=LAG(SW4); SW6=LAG(SW5); SW7=LAG(SW6);
SW8=LAG(SW7); SW9=LAG(SW8); SW10=LAG(SW9); SW11=LAG(SW10); SW12=LAG(SW11); SDZ=LAG12(Z);

PROC REG DATA=MSF;
MODEL SW=SDZ SW1 SW2 SW3 SW4 SW5 SW6 SW7 SW8 SW9 SW10 SW11 SW12;
OUTPUT OUT=OUT2 RESIDUAL=R;
PROC ARIMA DATA=OUT2;
IDENTIFY VAR=R;
RUN;

/* For seasonal unit root of X(1) */
Z=log(X);
W=DIF12(Z);
SW=DIF(W);
DX=LAG(W);
SW1=LAG(SW); SW2=LAG(SW1); SW3=LAG(SW2); SW4=LAG(SW3); SW5=LAG(SW4); SW6=LAG(SW5); SW7=LAG(SW6);
SW8=LAG(SW7); SW9=LAG(SW8); SW10=LAG(SW9); SW11=LAG(SW10); SW12=LAG(SW11);
PROC REG DATA=MSF;
MODEL SW=DX SW1 SW2 SW3 SW4 SW5 SW6 SW7 SW8 SW9 SW10 SW11 SW12;
OUTPUT OUT=OUT3 RESIDUAL=R;
PROC ARIMA DATA=OUT3;
IDENTIFY VAR=R;
RUN;

/* For seasonal unit root of X(12) */
Z=log(X)
W=DIF(Z);
SW=DIF12(W);
DX=LAG12(W);
...
PROC REG DATA=MSF;
MODEL SW=DX SW1 SW2 SW3 SW4 SW5 SW6 SW7 SW8 SW9 SW10 SW11 SW12;
OUTPUT OUT=OUT4 RESIDUAL=R;
PROC ARIMA DATA=OUT4;
IDENTIFY VAR=R;
RUN;

/*ARIMA modeling on the residuals*/
PROC ARIMA DATA=MSF;
IDENTIFY VAR=Z(12) ESACF MINIC STATIONARITY=(ADF=12);
IDENTIFY VAR=Z(1,12) ESACF MINIC STATIONARITY=(ADF=12);
IDENTIFY VAR=Z(1,12) ESACF MINIC STATIONARITY=(ADF=12 DLAG=12);
RUN;


/*Applying log and first dif to make the series stationary and then identifying and estimating models for fitting*/
DATA MSF;
INPUT Date $ X Y;
Z=log(X);
W=dif(Z);
...
…………………………
PROC ARIMA DATA=MSF;
IDENTIFY VAR=W ESACF MINIC NLAG=36 STATIONARITY=(ADF);
ESTIMATE P=1 Q=0 METHOD=ML PLOT;
ESTIMATE P=0 Q=1 METHOD=ML PLOT;
